...
interface TextUrlExtractorProps {
  ...
  onMetadata?: (
    metadata: Map<string, UrlMetadata & { isLoading: boolean }>
  ) => void;
}
...
const TextUrlExtractor: React.FC<TextUrlExtractorProps> = ({
  ...
  const [urlMetadata, setUrlMetadata] = useState<
    Map<string, UrlMetadata & { isLoading: boolean }>
  >(new Map());
  ...
  // Función para verificar si una URL es realmente válida (sin errores de metadata)
  const isUrlValidWithoutErrors = useCallback((urlObj: ExtractedUrl): boolean => {
    if (!urlObj.isValid) return false;
    
    const metadata = urlMetadata.get(urlObj.url);
    // Si hay metadata y tiene error, la URL no es válida
  ...
  const analyzeUrls = useCallback(
    async (urls: string[]) => {
      ...
      // Marcar todas las URLs como cargando
      urls.forEach((url) => {
        newMetadata.set(url, {
          url,
          statusCode: 0,
  ...
  useEffect(() => {
    ...
    // Notificar URLs válidas al componente padre (sin errores de metadata)
    const validUrlStrings = urls
      .filter((urlObj) => isUrlValidWithoutErrors(urlObj))
      .map((urlObj) => urlObj.url);

    if (onUrlsChange) {
      onUrlsChange(validUrlStrings);
    }
  }, [text, extractUrls, onUrlsChange, isUrlValidWithoutErrors]);
  ...
  // Efecto para re-notificar URLs válidas cuando cambian los metadatos
  useEffect(() => {
    if (extractedUrls.length > 0) {
      const validUrlStrings = extractedUrls
        .filter((urlObj) => isUrlValidWithoutErrors(urlObj))
        .map((urlObj) => urlObj.url);
  ...
  }, [urlMetadata, extractedUrls, isUrlValidWithoutErrors, onUrlsChange]);
  ...
  const removeUrl = (urlId: string) => {
    ...
    if (onUrlsChange) {
      onUrlsChange(updatedUrls.filter((u) => isUrlValidWithoutErrors(u)).map((u) => u.url));
    }
  };

  const validUrls = extractedUrls.filter((u) => isUrlValidWithoutErrors(u));
  const invalidUrls = extractedUrls.filter((u) => !isUrlValidWithoutErrors(u));
              ...
              <div className="flex items-center space-x-2">
                ...
                {(() => {
                  const totalCharacters = validUrls.reduce((total, urlObj) => {
                    const metadata = urlMetadata.get(urlObj.url);
                    // Solo contar caracteres de URLs válidas sin errores
                    if (metadata && !metadata.error) {
            ...
            <AccordionContent className="px-4 pb-4">
              <div className="space-y-2">
                ...
                {validUrls.reverse().map((urlObj) => {
                  ...
                  const hasMetadata =
                    metadata && !isLoading && metadata.isValid;
                        ...
                        <div className="p-4">
                          <div className="flex gap-4">
                            ...
                            <div className="flex-shrink-0">
                              {metadata.image ||
                              (metadata.isTwitter && metadata.author_image) ? (
                                <img
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  {(metadata?.complete_text ||
                                    ...
                                    <Badge
                                      variant="outline"
                                      className="bg-gray-50 text-gray-700 border-gray-300 shadow-sm"
                                    >
                        ...
                        <div className="p-4">
                          <div className="flex gap-4 items-center py-4">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-red-500 line-clamp-2 mb-3">
                                {metadata.error}
                              </p>
                ...
                {invalidUrls.map((urlObj) => (
                  ...
                  >
                    ...
                    {/* Content Area - Two Column Layout (same as valid URLs) */}
                    <div className="p-4">
                      <div className="flex gap-4 items-center py-4">
                        <div className="flex-shrink-0">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-red-500 line-clamp-2 mb-3 font-mono break-all">
                            {urlObj.url}
                          </p>
