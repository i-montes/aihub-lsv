...
export function PasoConsulta({
  ...
  return (
    <details
      className={cn(
        "group my-2 rounded-lg border text-base",
        error ? "border-red-200 bg-red-50" : "border-gray-200 bg-white"
      )}
    >
      <summary className="flex cursor-pointer select-none items-center gap-2 px-3 py-2 text-gray-600 [&::-webkit-details-marker]:hidden">
        {!terminado ? (
        ...
        <span className="shrink-0 text-base text-gray-400 group-open:hidden">ver SQL</span>
        <span className="hidden shrink-0 text-base text-gray-400 group-open:inline">ocultar</span>
      </summary>
      <div className="border-t border-gray-200/70 px-3 py-2.5">
        {input?.sql ? (
          <pre className="whitespace-pre-wrap break-words font-mono text-base leading-relaxed text-gray-700">
            {input.sql}
          </pre>
        ) : (
        ...
        {error && (
          <p className="mt-2 flex items-start gap-1.5 text-base leading-snug text-red-700">
